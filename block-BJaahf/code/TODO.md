...
You can use normal for loop for this function
     ...
     function loop(initialVal, condition, cb, console) {
  for(let i=initialVal; condition(i); i=cb(i) ){
    console(i)
  }
}
...
for (let i =0; i<arrays.length;i++){
  let arr2 =arrays[i];
  arr = arr.filter((elm)=>arr2.includes(elm));
}
return arr
}
...
for (let i =0; i<arrays.length;i++){
  let arr2 =arrays[i];
  arr = arr.filter((elm)=> !arr2.includes(elm)).concat(arr2);
}
return arr
}
